...
NeuralAmpModeler::NeuralAmpModeler(const InstanceInfo &info)
  ...
  mLayoutFunc = [&](IGraphics *pGraphics) {
    ...
    auto namNavUp = [&, pGraphics](IControl *pCaller) {
      WDL_String l_oStartNAMNavUpResult =
          FolderBrowser(pGraphics).StartNAMNavUp();
      std::string l_oGetNAMResult = this->_GetNAM(l_oStartNAMNavUpResult);
      int l_iFinishNAMNavUpResult =
          FolderBrowser(pGraphics).FinishNAMNavUp(l_oGetNAMResult);
    ...
    auto namNavDown = [&, pGraphics](IControl *pCaller) {
      WDL_String l_oStartNAMNavDownResult =
          FolderBrowser(pGraphics).StartNAMNavDown();
      std::string l_oGetNAMResult = this->_GetNAM(l_oStartNAMNavDownResult);
      int l_iFinishNAMNavDownResult =
          FolderBrowser(pGraphics).FinishNAMNavDown(l_oGetNAMResult);
    ...
    auto irNavUp = [&, pGraphics](IControl *pCaller) {
      WDL_String l_oStartIRNavUpResult =
          FolderBrowser(pGraphics).StartIRNavUp();
      const dsp::wav::LoadReturnCode l_oGetIRResult =
          this->_GetIR(l_oStartIRNavUpResult);
      int l_iFinishIRNavUpResult =
          FolderBrowser(pGraphics).FinishIRNavUp(l_oGetIRResult);
    ...
    auto irNavDown = [&, pGraphics](IControl *pCaller) {
      WDL_String l_oStartIRNavDownResult =
          FolderBrowser(pGraphics).StartIRNavDown();
      const dsp::wav::LoadReturnCode l_oGetIRResult =
          this->_GetIR(l_oStartIRNavDownResult);
      int l_iFinishIRNavDownResult =
          FolderBrowser(pGraphics).FinishIRNavDown(l_oGetIRResult);
    ...
    pGraphics
        ->AttachControl(
            new IRolloverSVGButtonControl(
                modelArea.GetFromRight(iconWidth).GetPadded(-3.f).GetHShifted(
                    -(iconWidth)),
                namNavUp, upArrowSVG),
            250)
    ...
    pGraphics
        ->AttachControl(
            new IRolloverSVGButtonControl(
                irArea.GetFromRight(iconWidth).GetPadded(-3.f).GetHShifted(
                    -(iconWidth)),
                irNavUp, upArrowSVG),
            252)
